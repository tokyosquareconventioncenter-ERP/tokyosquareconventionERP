...
export const NewTransactionModal: React.FC<NewTransactionModalProps> = ({
  ...
  // Synchronize initial modal opening to default cash account if CASH method and active project
  React.useEffect(() => {
    if (isOpen) {
      ...
      // Refresh saved custom categories, monthly heads, and money sources
      setCustomCategories(getSavedCustomCategories());
      setMonthlyExpenseHeads(getSavedCustomMonthlyExpenseHeads());
      setMoneySources(getSavedMoneyReceivedSources());
    }
  }, [isOpen, selectedProjectId, accounts]);
  ...
  const handleSaveCustomSourceInline = () => {
    const trimmed = customSourceText.trim();
    if (!trimmed) return;
    const updated = saveCustomMoneyReceivedSource(trimmed);
    setMoneySources(updated);
    setSourceType(trimmed);
    ...
    try {
      if (activeTab === 'EXPENSE') {
        ...
        const result = addExpense({
          ...
          supplierId: (expenseType === 'MATERIAL' && supplierId) ? supplierId : undefined,
          contractorId: (expenseType === 'CONTRACTOR' && contractorId) ? contractorId : undefined,
          reference,
        ...
        const result = addMoneyReceived({
          date,
          receivedFrom: receivedFrom || (settings?.companyNameBn || settings?.companyName || 'কোম্পানি ফান্ড'),
          sourceType,
        ...
        if (!contractorId) {
          setErrorMsg(isBn ? 'অনুগ্রহ করে তালিকা থেকে একজন কন্ট্রাক্টর নির্বাচন করুন!' : 'Please select a contractor from the list!');
          return;
        }
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
          ...
          {isSuccess ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      {isCustomSourceInput ? (
                        <div className="flex items-center gap-1.5">
                          <input
                            ...
                            placeholder={isBn ? 'নতুন উৎসের নাম লিখুন...' : 'Type new source name...'}
                            autoFocus
                            className="flex-1 px-3 py-2 bg-white border-2 border-amber-500 rounded-xl text-sm font-semibold text-slate-900 focus:outline-hidden"
                          />
                          <button
                            type="button"
                            onClick={handleSaveCustomSourceInline}
                            disabled={!customSourceText.trim()}
                            className="p-2 bg-amber-500 hover:bg-amber-600 disabled:opacity-40 text-slate-950 font-bold rounded-xl shadow-xs transition cursor-pointer"
                          ...
                          <button
                            ...
                            className="p-2 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold rounded-xl transition cursor-pointer"
                        ...
                        <div className="relative">
                          <select
                            value={sourceType}
                            onChange={(e) => {
                              if (e.target.value === '__NEW_CUSTOM_SOURCE__') {
                                setIsCustomSourceInput(true);
                              } else {
                          ...
                          >
                            {moneySources.map((src) => (
                              <option key={src} value={src} className="bg-slate-900 text-white font-medium py-1">
                                {src}
                              </option>
                            ))}
                            <option value="__NEW_CUSTOM_SOURCE__" className="bg-amber-600 text-white font-bold py-1">
                              ➕ {isBn ? '+ তালিকায় নতুন উৎস যোগ করুন...' : '+ Add New Source...'}
                            </option>
                          </select>
                          <ChevronDown className="w-4 h-4 text-slate-500 absolute right-3 top-3 pointer-events-none" />
                        </div>
                    ...
                    <div className="relative">
                      ...
                      >
                        <option value="" className="bg-slate-900 text-slate-400 font-medium py-1">
                          {isBn ? '-- কন্ট্রাক্টর নির্বাচন করুন --' : '-- Select Contractor --'}
                        </option>
                        {contractors.map(c => (
                          <option key={c.id} value={c.id} className="bg-slate-900 text-white font-medium py-1">
                            👷 {c.name} ({c.type}) {c.projectName ? `[${c.projectName}]` : ''} — বাকি: ৳{c.dueAmount.toLocaleString()}
                          </option>
                        ))}
                      </select>
                      <ChevronDown className="w-4 h-4 text-slate-500 absolute right-3 top-3 pointer-events-none" />
                    ...
                    <div className="relative">
                      ...
                      >
                        {accounts.map(acc => {
                          ...
                          let projBal: number | null = null;
                          if (activeTab !== 'TRANSFER' && projectId && projectId !== 'company') {
                            const pIn = (moneyReceived || []).reduce((sum, r) => {
                              const rAcc = accounts.find(a => a.id === r.accountId);
                              const isMatch = isCash
                                ? (rAcc?.type === 'CASH' || r.paymentMethod === 'CASH')
                                : (rAcc?.type === 'BANK' || rAcc?.type === 'MOBILE_BANKING' || r.paymentMethod === 'BANK' || r.paymentMethod === 'ONLINE' || r.paymentMethod === 'MOBILE_BANKING');
                              if (!isMatch) return sum;
                              if (r.projectAllocations && r.projectAllocations.length > 0) {
                                const alloc = r.projectAllocations.find(a => a.projectId === projectId);
                                return sum + (alloc ? alloc.amount : 0);
                              }
                              return sum + (r.projectId === projectId ? r.amount : 0);
                            }, 0);
                            const pOut = (expenses || []).filter(e => !e.isDeleted && e.projectId === projectId).reduce((sum, e) => {
                              const eAcc = accounts.find(a => a.id === e.accountId);
                              const isMatch = isCash
                                ? (eAcc?.type === 'CASH' || e.paymentMethod === 'CASH')
                                : (eAcc?.type === 'BANK' || eAcc?.type === 'MOBILE_BANKING' || e.paymentMethod === 'BANK' || e.paymentMethod === 'ONLINE' || e.paymentMethod === 'MOBILE_BANKING');
                              if (!isMatch) return sum;
                              return sum + e.amount;
                            }, 0);
                            projBal = pIn - pOut;
                          ...
                          return (
                            <option key={acc.id} value={acc.id} className="bg-slate-900 text-white font-medium py-1">
                              {acc.type === 'CASH' ? '💵' : '🏦'} {acc.name} {projBal !== null ? `(প্রকল্প স্থিতি: ৳${projBal.toLocaleString()})` : `(ব্যালেন্স: ৳${acc.currentBalance.toLocaleString()})`}
                            </option>
                          );
                        })}
                      </select>
              ...
              {/* Real-time Account & Project Balance Deficit Guard & Notification */}
              {(() => {
                if (activeTab === 'RECEIVE') return null;
                
                // If it's TRANSFER tab, check fromAccountId
                const checkAccId = activeTab === 'TRANSFER' ? fromAccountId : accountId;
                const checkAcc = accounts.find(a => a.id === checkAccId);
                if (!checkAcc) return null;

                const isCash = checkAcc.type === 'CASH';
                const isProjectSpecific = activeTab !== 'TRANSFER' && !!projectId && projectId !== 'company';
                const projObj = isProjectSpecific ? projects.find(p => p.id === projectId) : null;
                const projName = projObj ? projObj.name : 'প্রকল্প';

                let projIn = 0;
                let projOut = 0;
                let curBalance = checkAcc.currentBalance || 0;

                if (isProjectSpecific) {
                  projIn = (moneyReceived || []).reduce((sum, r) => {
                    const rAcc = accounts.find(a => a.id === r.accountId);
                    const isMatch = isCash
                      ? (rAcc?.type === 'CASH' || r.paymentMethod === 'CASH')
                      : (rAcc?.type === 'BANK' || rAcc?.type === 'MOBILE_BANKING' || r.paymentMethod === 'BANK' || r.paymentMethod === 'ONLINE' || r.paymentMethod === 'MOBILE_BANKING');
                  ...
                  projOut = (expenses || []).filter(e => !e.isDeleted && e.projectId === projectId).reduce((sum, e) => {
                    const eAcc = accounts.find(a => a.id === e.accountId);
                    const isMatch = isCash
                      ? (eAcc?.type === 'CASH' || e.paymentMethod === 'CASH')
                      : (eAcc?.type === 'BANK' || eAcc?.type === 'MOBILE_BANKING' || e.paymentMethod === 'BANK' || e.paymentMethod === 'ONLINE' || e.paymentMethod === 'MOBILE_BANKING');
                  ...
                  curBalance = projIn - projOut;
                }

                const numAmount = parseFloat(amount) || 0;
                const remaining = curBalance - numAmount;
                const isDeficit = numAmount > 0 && remaining < 0;

                return (
                  <div className="pt-1">
                    {isDeficit ? (
                      <div className="bg-rose-50 border-2 border-rose-400 rounded-2xl p-3.5 text-rose-950 flex items-start gap-3 shadow-xs">
                        ...
                        <div className="space-y-1 flex-1">
                          <div className="flex flex-wrap items-center justify-between gap-1.5">
                            <span className="font-black text-rose-900 text-xs sm:text-sm flex items-center gap-1.5">
                              <span>
                                ⚠️ {isBn 
                                  ? (isProjectSpecific 
                                      ? `সতর্কবার্তা: ${projName} প্রকল্পে অপর্যাপ্ত ${isCash ? 'নগদ' : 'ব্যাংক'} তহবিল!` 
                                      : 'সতর্কবার্তা: একাউন্টে অপর্যাপ্ত তহবিল (ঘাটতি সতর্কতা)!')
                                  : `Warning: Insufficient ${isCash ? 'Cash' : 'Bank'} in ${projName}!`}
                              </span>
                            </span>
                            <span className="bg-rose-200 border border-rose-400 text-rose-950 px-2 py-0.5 rounded-lg font-mono font-black text-xs">
                              {isBn ? `ঘাটতি হবে: -৳${Math.abs(remaining).toLocaleString()}` : `Deficit: -৳${Math.abs(remaining).toLocaleString()}`}
                            </span>
                          </div>
                          <p className="text-xs text-rose-900 leading-relaxed font-medium">
                            {isBn ? (
                              isProjectSpecific ? (
                                <>
                                  <strong>{projName}</strong> প্রকল্পে বর্তমানে {isCash ? '💵 নগদ জমা' : '🏦 ব্যাংক জমা'} স্থিতি আছে <strong className="font-mono">৳{curBalance.toLocaleString()}</strong> (মোট প্রাপ্তি: ৳{projIn.toLocaleString()}, মোট খরচ: ৳{projOut.toLocaleString()})। আপনি খরচ করছেন <strong className="font-mono text-rose-700">৳{numAmount.toLocaleString()}</strong>। এই এন্ট্রি সেভ করলে এই প্রকল্পে <strong className="font-mono text-rose-700">-৳{Math.abs(remaining).toLocaleString()}</strong> ঘাটতি তৈরি হবে!
                                  <span className="block mt-1 text-[11px] text-slate-600 font-medium">
                                    💡 প্রধান {checkAcc.name} এ কোম্পানির সর্বমোট জমা আছে: ৳{checkAcc.currentBalance.toLocaleString()}
                                  </span>
                                </>
                              ) : (
                                <>
                                  <strong>{checkAcc.name}</strong> এ বর্তমানে জমা আছে <strong className="font-mono">৳{curBalance.toLocaleString()}</strong>। আপনি খরচ বা স্থানান্তর করছেন <strong className="font-mono text-rose-700">৳{numAmount.toLocaleString()}</strong>। এই এন্ট্রি সেভ করলে একাউন্টে <strong className="font-mono text-rose-700">-৳{Math.abs(remaining).toLocaleString()}</strong> ঘাটতি তৈরি হবে!
                                </>
                              )
                            ) : (
                              <>
                                <strong>{isProjectSpecific ? projName : checkAcc.name}</strong> currently has <strong className="font-mono">৳{curBalance.toLocaleString()}</strong>. Spending <strong className="font-mono text-rose-700">৳{numAmount.toLocaleString()}</strong> results in a deficit of <strong className="font-mono text-rose-700">-৳{Math.abs(remaining).toLocaleString()}</strong>.
                              </>
                            )}
                          </p>
                        </div>
                      ...
                      <div className="bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 flex flex-wrap items-center justify-between gap-2 text-xs">
                        <div className="flex items-center gap-2 text-slate-700 font-semibold">
                          <span>{isCash ? '💵' : '🏦'}</span>
                          <span>
                            {isProjectSpecific ? (
                              <>
                                <strong className="text-slate-900">{projName}</strong> ({isCash ? 'নগদ স্থিতি' : 'ব্যাংক স্থিতি'}):
                              </>
                            ) : (
                              <>
                                <strong>{checkAcc.name}</strong>:
                              </>
                            )}
                          </span>
                          <span className={`font-mono font-bold ${curBalance < 0 ? 'text-rose-600' : 'text-slate-900'}`}>
                            ৳{curBalance.toLocaleString()}
                          </span>
                          {isProjectSpecific && (
                            <span className="text-[10px] text-slate-500 font-normal">
                              (প্রধান ফান্ড: ৳{checkAcc.currentBalance.toLocaleString()})
                            </span>
                          )}
                        </div>
                        {numAmount > 0 ? (
                          <div className="flex items-center gap-1.5 font-mono text-emerald-800 bg-emerald-100/80 px-2.5 py-1 rounded-lg text-[11px] font-bold">
                            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                            <span>
                              {isBn 
                                ? (isProjectSpecific 
                                    ? `এই প্রকল্পে খরচ বাদে অবশিষ্ট থাকবে: ৳${remaining.toLocaleString()}` 
                                    : `খরচ বাদে অবশিষ্ট থাকবে: ৳${remaining.toLocaleString()}`)
                                : `Remaining: ৳${remaining.toLocaleString()}`}
                            </span>
                          </div>
                        ) : (
                          <span className="text-[11px] text-slate-500 font-medium">
                            {isBn ? 'খরচের টাকা লিখলে অবশিষ্ট ব্যালেন্স দেখতে পাবেন' : 'Enter amount to see projected balance'}
                          </span>
                        )}
                      </div>
                    )}
