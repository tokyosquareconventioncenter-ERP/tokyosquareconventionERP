      ...
      >
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className={`font-bold text-xs sm:text-sm leading-tight truncate ${
                ...
                {isBn 
                  ? (settings?.companyNameBn || settings?.companyName || t('companyShortName'))
                  : (settings?.companyName || settings?.companyNameBn || t('companyShortName'))}
              </h1>
              <p className={`text-[10px] font-mono tracking-wider truncate ${
                isDark ? 'text-amber-400' : 'text-amber-600 font-bold'
              }`}>
                {isBn ? (settings?.erpTitleBn || 'কনস্ট্রাকশন ইআরপি') : (settings?.erpTitle || 'CONSTRUCTION ERP')}
              </p>
            </div>
          </div>
          <button
        ...
        <div className={`flex-1 overflow-y-auto px-3 py-3 space-y-0.5 scrollbar-thin ${
          ...
          {(role === 'SUPER_ADMIN' || role === 'ADMIN') && (
            <>
              {navSection('সিস্টেম ও কনফিগারেশন', 'Admin & Settings')}
              {hasPermission(role, 'canManageSettings') && (
                navItem('SETTINGS', 'সিস্টেম সেটিংস', 'ERP Settings', <Settings className="w-4 h-4 text-amber-400" />)
              )}
