...
export function formatCurrency(amount: number, lang: Language = 'en'): string {
  ...
  // Use non-breaking space (\u00A0) so currency symbol and amount never split across lines
  const nbsp = '\u00A0';
  if (lang === 'bn') {
    return isNegative ? `-৳${nbsp}${toBanglaDigits(formatted)}` : `৳${nbsp}${toBanglaDigits(formatted)}`;
  }
  return isNegative ? `-৳${nbsp}${formatted}` : `৳${nbsp}${formatted}`;
}
