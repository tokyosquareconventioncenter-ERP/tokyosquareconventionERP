...
export const LedgerView: React.FC = () => {
  ...
  // Filter-aware 4th Card Calculation:
  // When a specific month is chosen: use that month's closingBalance
  // When 'ALL' months: totalCredits - totalDebits represents the exact net position
  const filterAwareClosingBalance = useMemo(() => {
    if (selectedMonth !== 'ALL') {
      const currentMonthGroup = groupedByMonth.find(g => g.monthKey === selectedMonth);
      return currentMonthGroup ? currentMonthGroup.closingBalance : 0;
    }
    return totalCredits - totalDebits;
  }, [selectedMonth, groupedByMonth, totalCredits, totalDebits]);

  const fourthCardLabel = useMemo(() => {
    if (filterProject !== 'ALL') {
      const pName = projects.find(p => p.id === filterProject)?.name;
      return isBn 
        ? `${pName || 'প্রকল্প'} সমাপনী স্থিতি` 
        : 'Project Net Closing Balance';
    }
    if (filterAccount !== 'ALL') {
      const aName = accounts.find(a => a.id === filterAccount)?.name;
      return isBn 
        ? `${aName || 'অ্যাকাউন্ট'} সমাপনী স্থিতি` 
        : 'Account Net Closing Balance';
    }
    return isBn 
      ? (selectedMonth === 'ALL' ? 'লেজার সমাপনী স্থিতি (নেট ব্যালেন্স)' : 'নির্বাচিত মাসের সমাপনী স্থিতি') 
      : 'Net Closing Balance';
  }, [filterProject, filterAccount, selectedMonth, projects, accounts, isBn]);
    ...
    <div className="space-y-6">
      ...
      <div className={`border rounded-2xl p-5 shadow-sm space-y-4 transition ${
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 pt-1">
          ...
          <div className={`border p-3 rounded-xl transition ${
            isDark 
              ? (filterAwareClosingBalance < 0 ? 'bg-slate-900/80 border-rose-500/40' : 'bg-slate-900/80 border-emerald-500/30')
              : (filterAwareClosingBalance < 0 ? 'bg-rose-50/70 border-rose-300' : 'bg-emerald-50/60 border-emerald-200')
          }`}>
            <span className={`text-[11px] block font-medium truncate ${
              isDark 
                ? (filterAwareClosingBalance < 0 ? 'text-rose-300' : 'text-emerald-300') 
                : (filterAwareClosingBalance < 0 ? 'text-rose-700' : 'text-emerald-700')
            }`}>
              {fourthCardLabel}
            </span>
            <span className={`text-base sm:text-lg font-mono font-black mt-0.5 block ${
              isDark 
                ? (filterAwareClosingBalance < 0 ? 'text-rose-400' : 'text-emerald-300') 
                : (filterAwareClosingBalance < 0 ? 'text-rose-600' : 'text-emerald-700')
            }`}>
              {formatCurrency(filterAwareClosingBalance, isBn ? 'bn' : 'en')}
            </span>
            <span className={`text-[10px] font-bold block mt-0.5 ${
              filterAwareClosingBalance < 0 ? 'text-rose-500' : 'text-emerald-600'
            }`}>
              {filterAwareClosingBalance < 0 
                ? (isBn ? '⚠️ ঘাটতি / বকেয়া' : 'Deficit / Due') 
                : (isBn ? '✅ ব্যালেন্স উদ্বৃত্ত' : 'Surplus')}
            </span>
          </div>
        </div>
      </div>
                ...
                <table className="w-full text-left text-xs ledger-print-table">
                  <thead>
                    <tr className={`border-b uppercase text-[10px] tracking-wider ${
                      ...
                      <th className={`py-3 px-4 text-center print:hidden sticky right-0 z-20 shadow-[-6px_0_10px_-2px_rgba(0,0,0,0.08)] border-l ${
                        isDark ? 'bg-slate-900 border-slate-700 text-slate-300' : 'bg-slate-100 border-slate-200 text-slate-700'
                      }`}>{isBn ? 'অ্যাকশন' : 'Action'}</th>
                    </tr>
                  </thead>
                    ...
                    {group.entries.map((entry) => {
                      ...
                      return (
                        <tr key={entry.id} className={`transition ${
                          ...
                          <td className={`py-3 px-4 text-center whitespace-nowrap print:hidden sticky right-0 z-10 shadow-[-6px_0_10px_-2px_rgba(0,0,0,0.08)] border-l transition ${
                            isDark 
