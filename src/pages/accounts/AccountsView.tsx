...
import { AccountType, Account, Voucher } from '../../types';
...
export const AccountsView: React.FC<AccountsViewProps> = ({ onOpenNewTransaction, onOpenTransfer }) => {
  ...
  const { accounts, ledger, vouchers, addAccount, updateAccount, deleteAccount, deleteAccountTransfer } = useData();
  ...
  const [transferToDelete, setTransferToDelete] = useState<Voucher | null>(null);
  const [transferDeleteReason, setTransferDeleteReason] = useState('');
  ...
  // Filter all Contra / Internal transfers
  const transferVouchers = React.useMemo(() => {
    return vouchers
      .filter(v => v.transactionType === 'TRANSFER' || v.voucherNumber?.startsWith('CTV-'))
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }, [vouchers]);

  const handleConfirmDeleteTransfer = () => {
    if (!transferToDelete) return;
    deleteAccountTransfer(transferToDelete.voucherNumber || transferToDelete.id, transferDeleteReason || 'Deleted from Accounts View');
    setNotice(isBn 
      ? `ফান্ড ট্রান্সফার (${transferToDelete.voucherNumber}) সফলভাবে মুছে ফেলা হয়েছে এবং ব্যালেন্স পূর্বাবস্থায় ফিরিয়ে দেওয়া হয়েছে।` 
      : `Fund transfer (${transferToDelete.voucherNumber}) deleted successfully and balances restored.`
    );
    setTransferToDelete(null);
    setTransferDeleteReason('');
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Internal Transfers / Contra Register Section */}
      <div className={`mt-8 rounded-2xl border transition overflow-hidden ${
        ...
        <div className={`p-5 border-b flex flex-wrap items-center justify-between gap-4 ${
          isDark ? 'border-slate-700/80 bg-slate-900/40' : 'border-slate-200 bg-slate-50/50'
        }`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className={`text-base font-bold flex items-center gap-2 ${
                ...
                <span>{isBn ? 'অভ্যন্তরীণ ফান্ড ট্রান্সফার হিস্ট্রি (কন্ট্রা রেজিস্টার)' : 'Internal Fund Transfers & Contra Register'}</span>
                <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${
                  isDark ? 'bg-teal-500/20 text-teal-300' : 'bg-teal-100 text-teal-800'
                }`}>
                  {transferVouchers.length}
                </span>
              </h3>
              <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                {isBn 
                  ? 'ব্যাংক ⇄ ক্যাশ ব্যালেন্স স্থানান্তর রেকর্ড। ভুল হলে যেকোনো ট্রান্সফার ১-ক্লিকে ডিলিট করে ব্যালেন্স রিভার্স করুন।' 
                  : 'Bank ⇄ Cash transfer log. Delete any faulty transfer with 1-click cascading balance reversal.'}
              </p>
            </div>
          </div>
          ...
          <button
            type="button"
            onClick={() => onOpenTransfer ? onOpenTransfer() : onOpenNewTransaction('TRANSFER')}
            className="flex items-center gap-2 px-4 py-2 bg-teal-600 hover:bg-teal-500 text-white rounded-xl text-xs font-bold transition shadow-sm cursor-pointer"
          >
            <Plus className="w-4 h-4" />
            <span>{isBn ? 'নতুন ফান্ড ট্রান্সফার' : 'New Fund Transfer'}</span>
          </button>
        </div>
        ...
        {transferVouchers.length === 0 ? (
          <div className="p-8 text-center">
            <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
              {isBn ? 'এখনও কোনো অভ্যন্তরীণ ফান্ড ট্রান্সফার (Contra) এন্ট্রি করা হয়নি।' : 'No internal fund transfers recorded yet.'}
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className={`border-b font-bold uppercase tracking-wider ${
                isDark ? 'bg-slate-900/60 border-slate-700/80 text-slate-400' : 'bg-slate-50 border-slate-200 text-slate-600'
              }`}>
                <tr>
                  <th className="py-3 px-4">{isBn ? 'তারিখ' : 'Date'}</th>
                  <th className="py-3 px-4">{isBn ? 'ভাউচার নম্বর' : 'Voucher No'}</th>
                  <th className="py-3 px-4">{isBn ? 'প্রেরক ➔ প্রাপক অ্যাকাউন্ট' : 'Transfer Route'}</th>
                  <th className="py-3 px-4 text-right">{isBn ? 'পরিমাণ' : 'Amount'}</th>
                  <th className="py-3 px-4">{isBn ? 'বিবরণ ও রেফারেন্স' : 'Description'}</th>
                  ...
                  <tr key={v.id} className={`transition ${
                    ...
                    <td className="py-3 px-4 whitespace-nowrap">
                      <span className={`px-2 py-0.5 rounded-md font-mono font-bold text-[11px] ${
                        isDark ? 'bg-teal-500/20 text-teal-300' : 'bg-teal-50 text-teal-700 border border-teal-200'
                      }`}>
                        {v.voucherNumber}
                      </span>
                    </td>
                    <td className="py-3 px-4 font-medium">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <span className={isDark ? 'text-slate-300' : 'text-slate-800'}>
                          {v.accountName || (isBn ? 'প্রেরক অ্যাকাউন্ট' : 'Source A/C')}
                        </span>
                        <span className="text-teal-500 font-bold">➔</span>
                        <span className={isDark ? 'text-emerald-400' : 'text-emerald-700'}>
                          {v.paidTo || (isBn ? 'প্রাপক অ্যাকাউন্ট' : 'Destination A/C')}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className={`py-3 px-4 max-w-xs truncate ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                      {v.description || v.reference || '—'}
                    </td>
                    <td className="py-3 px-4 text-center whitespace-nowrap">
                      <button
                        type="button"
                        onClick={() => setTransferToDelete(v)}
                        title={isBn ? 'ফান্ড ট্রান্সফার ডিলিট ও ব্যালেন্স রিভার্স করুন' : 'Delete Transfer & Reverse Balances'}
                        className={`p-1.5 rounded-lg transition cursor-pointer ${
                          isDark 
      ...
      {/* Delete Internal Transfer / Contra Modal */}
      {transferToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-xs p-3">
          <div className="bg-white text-slate-900 rounded-2xl shadow-2xl border border-slate-200 w-full max-w-md p-6 space-y-4">
            <div className="flex items-center gap-3 text-rose-600">
              ...
              <div>
                <h3 className="font-bold text-base text-slate-900">{isBn ? 'ফান্ড স্থানান্তর (Contra) মুছে ফেলবেন?' : 'Delete Fund Transfer?'}</h3>
                <span className="text-xs font-mono font-bold text-rose-600">{transferToDelete.voucherNumber}</span>
              </div>
            </div>

            <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 text-xs text-amber-900 space-y-1 leading-relaxed">
              <p className="font-bold flex items-center gap-1.5 text-amber-950">
                <span>⚠️ {isBn ? 'স্বয়ংক্রিয় ব্যালেন্স রিভার্সাল ও ক্লিনিং:' : 'Automatic Balance Reversal & Cleanup:'}</span>
              </p>
              <p>
                {isBn 
                  ? `আপনি কি নিশ্চিত যে ভাউচার নং ${transferToDelete.voucherNumber} (৳${transferToDelete.amount.toLocaleString()}) মুছে ফেলতে চান? এটি মুছে ফেললে:`
                  : `Are you sure you want to delete Transfer ${transferToDelete.voucherNumber} (৳${transferToDelete.amount.toLocaleString()})? This will:`}
              </p>
              <ul className="list-disc list-inside space-y-0.5 text-amber-800 pt-1">
                <li>{isBn ? 'প্রেরক অ্যাকাউন্টে টাকা ফেরত যোগ হবে।' : 'Restore money back to the Source Account.'}</li>
                <li>{isBn ? 'প্রাপক অ্যাকাউন্ট থেকে টাকা কেটে সমন্বয় করা হবে।' : 'Deduct money back from the Destination Account.'}</li>
                <li>{isBn ? 'ভাউচার রেজিস্টার এবং জেনারেল লেজার উভয় পাশ থেকেই রেকর্ডগুলো সম্পূর্ণ মুছে যাবে।' : 'Cleanly delete both debit and credit entries from Vouchers and General Ledger.'}</li>
              </ul>
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                value={transferDeleteReason}
                onChange={(e) => setTransferDeleteReason(e.target.value)}
                placeholder={isBn ? 'যেমন: ভুল অ্যাকাউন্টে ট্রান্সফার হয়েছিল' : 'e.g., Wrong account transfer'}
            ...
            <div className="flex justify-end gap-3 pt-3 border-t border-slate-200">
              ...
              >
                {isBn ? 'হ্যাঁ, ট্রান্সফার ডিলিট ও রিভার্স করুন' : 'Yes, Delete & Reverse'}
              </button>
            </div>
          </div>
        </div>
