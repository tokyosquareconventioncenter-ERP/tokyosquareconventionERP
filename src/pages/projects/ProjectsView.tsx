...
export const ProjectsView: React.FC = () => {
  ...
  const { projects, addProject, updateProject, deleteProject, expenses, moneyReceived, settings } = useData();
  ...
  const openEditProjectModal = (p: Project) => {
    ...
    setEditName(p.name || '');
    setEditCode(p.code || '');
    setEditClient(p.client || settings?.companyName || '');
  ...
  const [client, setClient] = useState(settings?.companyName || '');
  ...
  const handleCreateProject = (e: React.FormEvent) => {
    ...
    addProject({
      ...
      client: client || settings?.companyName || '',
      location,
      startDate,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredProjects.map((project) => {
          ...
          const liveMaterialCost = projectExpenses
            .filter(e => (e.expenseType === 'MATERIAL' || (e.supplierId && e.supplierId !== 'NONE') || e.category?.toLowerCase().includes('material') || e.category?.toLowerCase().includes('নির্মাণ')) && (!e.contractorId || e.contractorId === 'NONE'))
          ...
          const liveContractorCost = projectExpenses
            .filter(e => e.expenseType === 'CONTRACTOR' || (e.contractorId && e.contractorId !== 'NONE') || e.category?.toLowerCase().includes('contractor') || e.category?.toLowerCase().includes('কন্ট্রাক্টর'))
            .reduce((s, e) => s + e.amount, 0);
