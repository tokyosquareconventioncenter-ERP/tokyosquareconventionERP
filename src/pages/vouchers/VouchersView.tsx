        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-xs p-3">
          <div className="bg-white text-slate-900 rounded-2xl shadow-2xl border border-slate-200 w-full max-w-md p-6 space-y-4">
            ...
            <p className="text-xs text-slate-600 leading-relaxed">
              {deleteModalVoucher.voucherNumber?.startsWith('CTV-') || deleteModalVoucher.transactionType === 'TRANSFER'
                ? (isBn 
                    ? `⚠️ এটি একটি অভ্যন্তরীণ ফান্ড স্থানান্তর (Contra)। এটি মুছে ফেললে প্রেরক ও প্রাপক অ্যাকাউন্টের ব্যালেন্স সাথে সাথে পূর্বাবস্থায় ফিরে যাবে এবং জেনারেল লেজার থেকেও উভয় পাশের এন্ট্রি স্বয়ংক্রিয়ভাবে মুছে যাবে। আপনি কি নিশ্চিত?`
                    : `⚠️ This is an internal fund transfer (Contra). Deleting this will instantly restore balances in both Bank/Cash accounts and remove both entries from General Ledger. Proceed?`)
                : (isBn 
                    ? `আপনি কি নিশ্চিত যে ভাউচার নং ${deleteModalVoucher.voucherNumber} (৳${deleteModalVoucher.amount.toLocaleString()}) মুছে ফেলতে চান? এটি ক্যাশ/ব্যাংক ব্যালেন্স সমন্বয় করবে এবং লেজার ও সংশ্লিষ্ট খতিয়ান থেকে স্বয়ংক্রিয়ভাবে মুছে যাবে।`
                    : `Are you sure you want to delete Voucher ${deleteModalVoucher.voucherNumber} (৳${deleteModalVoucher.amount.toLocaleString()})? This will restore account balance and cascade delete all linked records.`)}
            </p>
