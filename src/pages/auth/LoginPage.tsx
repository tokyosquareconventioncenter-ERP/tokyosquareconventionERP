...
export const LoginPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col justify-between bg-slate-950 text-slate-100 selection:bg-amber-500 selection:text-slate-950 relative overflow-hidden">
        ...
        >
          ...
          <div className="text-center space-y-2">
            ...
            <h1 className="text-2xl font-extrabold tracking-tight text-slate-900">
              {isBn 
                ? (settings?.companyNameBn || settings?.companyName || 'কনস্ট্রাকশন ইআরপি লগইন') 
                : (settings?.companyName || settings?.companyNameBn || 'Construction ERP Login')}
            </h1>
            <p className="text-xs sm:text-sm text-slate-500">
              {isBn 
                ? (settings?.erpTitleBn || settings?.erpTitle || 'আপনার ইমেইল ও পাসওয়ার্ড দিয়ে একাউন্টে প্রবেশ করুন') 
                : (settings?.erpTitle || settings?.erpTitleBn || 'Sign in with your authorized email and password')}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <Input
              ...
              placeholder={isBn ? 'যেমন: admin@company.com' : 'e.g. admin@company.com'}
              value={email}
      ...
      {/* Footer */}
      <footer className="relative z-10 w-full py-6 text-center text-xs text-slate-500 space-y-1">
        <p>© {new Date().getFullYear()} {isBn ? 'সর্বস্বত্ব সংরক্ষিত — কনস্ট্রাকশন ইআরপি সলিউশন' : 'Construction ERP Solution — All rights reserved.'}</p>
        <p className="text-[11px] text-amber-500/80">
          {isBn 
            ? 'পাওয়ার্ড বাই: আবাবিল সফটওয়্যার সলিউশনস (ইঞ্জিনিয়ার মো. তানভীন আহমেদ টুটুল | বিকাশ: 01672965561)' 
