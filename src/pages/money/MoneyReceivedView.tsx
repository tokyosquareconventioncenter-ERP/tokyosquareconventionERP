...
export const MoneyReceivedView: React.FC<MoneyReceivedViewProps> = ({ 
  ...
  const normalizeName = (name: string): string => {
    if (!name) return (settings?.companyNameBn || settings?.companyName || 'General Owner / Company');
    const trimmed = name.trim().replace(/\s+/g, ' ');
    const lower = trimmed.toLowerCase();
    
    // Normalized mapping for well-known contributors & lenders
    if (lower.includes('arifa') || lower.includes('ম্যাডাম') || lower.includes('madam')) {
      return 'Director Arifa Akhter';
    }
    if (lower.includes('k.r islam') || lower.includes('k.r. islam') || lower.includes('কে আর ইসলাম') || lower.includes('ডঃ কে আর ইসলাম') || lower.includes('kayer islam') || lower.includes('kr islam')) {
      return 'Dr. K.R ISLAM Sir';
    }
    if (lower.includes('hamid') || lower.includes('md. abdul hamid') || lower.includes('abdul hamid') || lower.includes('হামিদ')) {
      return 'Hamid Sir';
    }
    if (lower.includes('raihan') || lower.includes('ruhul amin') || lower.includes('রায়হান')) {
      return 'Raihan Sir';
    }
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className={`border-b uppercase text-[10px] tracking-wider ${
                isDark ? 'border-slate-700 bg-slate-900/90 text-slate-400' : 'border-slate-200 bg-slate-100 text-slate-600'
              }`}>
                <th className="py-3 px-3 min-w-[130px]">{isBn ? 'রিসিট নং ও এডিট' : 'Receipt No & Edit'}</th>
                <th className="py-3 px-3 min-w-[95px]">{isBn ? 'তারিখ' : 'Date'}</th>
                <th className="py-3 px-3 min-w-[160px]">{isBn ? 'প্রকল্প' : 'Project'}</th>
                <th className="py-3 px-3 min-w-[180px]">{isBn ? 'প্রদানকারী (কার নিকট হতে প্রাপ্ত)' : 'Received From'}</th>
                <th className="py-3 px-3 min-w-[110px]">{isBn ? 'উৎস / খাত' : 'Source / Head'}</th>
                <th className="py-3 px-3 min-w-[140px]">{isBn ? 'জমা একাউন্ট' : 'Account'}</th>
                <th className="py-3 px-3 text-right min-w-[110px]">{isBn ? 'টাকার পরিমাণ' : 'Amount'}</th>
                <th className={`py-3 px-3 text-center min-w-[140px] sticky right-0 z-20 shadow-[-6px_0_10px_-2px_rgba(0,0,0,0.08)] border-l ${
                  isDark ? 'bg-slate-900 border-slate-700 text-slate-300' : 'bg-slate-100 border-slate-200 text-slate-700'
                }`}>
                  {isBn ? 'প্রিন্ট ও অ্যাকশন' : 'Print / Action'}
                </th>
              </tr>
            </thead>
                ...
                filtered.map((item) => (
                  ...
                  >
                    <td className="py-3 px-3 whitespace-nowrap">
                      <div className="flex items-center gap-1.5">
                        <span className={`font-mono font-bold px-2 py-0.5 rounded-md text-[11px] border ${
                          isDark 
                        ...
                        <button
                          ...
                          className={`p-1 rounded-md transition cursor-pointer ${
                            isDark 
                              ? 'bg-blue-500/20 hover:bg-blue-600 text-blue-300 hover:text-white' 
                              : 'bg-blue-50 hover:bg-blue-600 text-blue-700 hover:text-white border border-blue-200'
                          }`}
                          title={isBn ? 'সরাসরি এডিট করুন' : 'Quick Edit'}
                        >
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="flex flex-col gap-0.5 text-[10px]">
                            {item.projectAllocations.map((a, aIdx) => {
                              const isSelected = filterProject !== 'ALL' && a.projectId === filterProject;
                              return (
                                <span 
                                  key={aIdx} 
                                  className={`${
                                    isSelected 
                                      ? 'font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded border border-emerald-500/30' 
                                      : isDark ? 'text-slate-300' : 'text-slate-600'
                                  }`}
                                >
                                  {isSelected ? '👉 ' : '• '}{a.projectName}: <strong className="font-mono">৳{a.amount.toLocaleString()}</strong>
                                  {isSelected && <span className="text-[9px] font-semibold ml-1">({isBn ? 'এই প্রজেক্ট' : 'This'})</span>}
                                </span>
                              );
                            })}
                          </div>
                    ...
                    <td className="py-3 px-3 text-right whitespace-nowrap">
                      {(() => {
                        const alloc = (filterProject !== 'ALL' && item.projectAllocations && item.projectAllocations.length > 0)
                          ? item.projectAllocations.find(a => a.projectId === filterProject)
                          : null;
                        const displayAmt = alloc ? alloc.amount : item.amount;
                        return (
                          <div>
                            <span className="font-mono font-black text-sm text-emerald-500 block">
                              {formatCurrency(displayAmt, isBn ? 'bn' : 'en')}
                            </span>
                            {alloc ? (
                              <span className="text-[10px] text-amber-600 dark:text-amber-400 font-sans block mt-0.5 font-medium">
                                {isBn 
                                  ? `(প্রজেক্ট অংশ • মূল রশিদ: ৳${item.amount.toLocaleString()})` 
                                  : `(Project share • Total: ৳${item.amount.toLocaleString()})`}
                              </span>
                            ) : item.projectAllocations && item.projectAllocations.length > 1 ? (
                              <span className="text-[10px] text-slate-400 font-sans block mt-0.5">
                                {isBn ? `(${item.projectAllocations.length}টি প্রজেক্টে ভাগ)` : `(Split across ${item.projectAllocations.length} projects)`}
                              </span>
                            ) : null}
                          </div>
                        );
                    ...
                    <td className={`py-3 px-3 text-center whitespace-nowrap sticky right-0 z-10 shadow-[-6px_0_10px_-2px_rgba(0,0,0,0.08)] border-l transition ${
                      isDark 
                        ? 'bg-slate-800 group-hover:bg-slate-700/90 border-slate-700/60' 
                      ...
                      <div className="flex items-center justify-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1 px-2.5 py-1 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold rounded-lg transition text-xs shadow-xs cursor-pointer"
                        >
                          <Printer className="w-3.5 h-3.5" />
      ...
      {isReceiptRegisterPrintOpen && (
        ...
        >
          <div className="bg-white text-slate-950 rounded-2xl shadow-2xl border border-slate-300 w-full max-w-5xl max-h-[94vh] flex flex-col overflow-hidden print:border-none print:shadow-none print:max-w-none print:max-h-none print:w-full print:rounded-none print:overflow-visible print:block">
            ...
            <div className="p-6 sm:p-8 overflow-y-auto print:p-0 print:overflow-visible text-slate-950">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-100 p-3 rounded-lg border border-slate-400 text-xs mb-4">
                ...
                <div>
                  ...
                  <span className="font-black text-slate-950 text-sm text-emerald-800 whitespace-nowrap">{formatCurrency(totalReceived, isBn ? 'bn' : 'en')}</span>
                </div>
              </div>

              <div className="border-2 border-slate-800 rounded-lg overflow-hidden mb-6">
                <table className="w-full text-left text-xs border-collapse receipt-register-table table-fixed">
                  <thead>
                    <tr className="bg-slate-200 border-b-2 border-slate-800 text-slate-950 font-black text-[10px] sm:text-[11px] uppercase">
                      <th className="py-2 px-1 border-r border-slate-400 text-center w-[3.5%]">{isBn ? 'ক্র.' : 'Sl'}</th>
                      <th className="py-2 px-1.5 border-r border-slate-400 w-[11.5%] whitespace-nowrap">{isBn ? 'রিসিট নং' : 'Receipt No'}</th>
                      <th className="py-2 px-1 border-r border-slate-400 w-[9.5%] whitespace-nowrap">{isBn ? 'তারিখ' : 'Date'}</th>
                      <th className="py-2 px-1.5 border-r border-slate-400 w-[13.5%]">{isBn ? 'প্রকল্প' : 'Project'}</th>
                      <th className="py-2 px-1.5 border-r border-slate-400 w-[13%]">{isBn ? 'প্রদানকারী' : 'Received From'}</th>
                      <th className="py-2 px-1.5 border-r border-slate-400 w-[22%]">{isBn ? 'উৎস ও বিবরণ' : 'Source & Description'}</th>
                      <th className="py-2 px-1.5 border-r border-slate-400 w-[13%]">{isBn ? 'জমা একাউন্ট' : 'Account'}</th>
                      <th className="py-2 px-1.5 text-right w-[14%] whitespace-nowrap">{isBn ? 'পরিমাণ (৳)' : 'Amount (Tk)'}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y border-slate-300 text-[10.5px]">
                    {filtered.map((r, idx) => (
                      <tr key={r.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                        ...
                        <td className="py-1.5 px-1.5 border-r border-slate-300 font-mono font-bold text-slate-950 whitespace-nowrap">{r.receiptNumber || 'MR-GEN'}</td>
                        <td className="py-1.5 px-1 border-r border-slate-300 whitespace-nowrap">{formatDisplayDate(r.date, isBn ? 'bn' : 'en')}</td>
                        <td className="py-1.5 px-1.5 border-r border-slate-300 font-medium">
                          {filterProject !== 'ALL' && r.projectAllocations && r.projectAllocations.length > 1 ? (
                            <div>
                              <span className="font-bold text-slate-900">{selectedProjectName}</span>
                              <span className="text-[9px] text-slate-500 block">
                                {isBn ? 'মোট ৳' + r.amount.toLocaleString() + ' হতে প্রজেক্ট ভাগ' : 'Allocated from ৳' + r.amount.toLocaleString()}
                              </span>
                            </div>
                          ) : (
                        ...
                        <td className="py-1.5 px-1.5 border-r border-slate-300 font-bold text-slate-950">{r.receivedFrom}</td>
                        <td className="py-1.5 px-1.5 border-r border-slate-300">
                          <span className="font-bold text-slate-900 block">{r.sourceType}</span>
                          <span className="text-slate-600 block text-[9.5px]">{r.description}</span>
                        </td>
                        <td className="py-1.5 px-1.5 border-r border-slate-300 text-slate-700">{r.accountName}</td>
                        <td className="py-1.5 px-1.5 text-right font-mono font-black text-slate-950 whitespace-nowrap">
                          {(() => {
                            const alloc = (filterProject !== 'ALL' && r.projectAllocations && r.projectAllocations.length > 0)
                              ? r.projectAllocations.find(a => a.projectId === filterProject)
                              : null;
                            const printAmt = alloc ? alloc.amount : r.amount;
                            return (
                              <div>
                                <span className="whitespace-nowrap">{formatCurrency(printAmt, isBn ? 'bn' : 'en')}</span>
                                {alloc && (
                                  <span className="text-[9px] text-slate-500 font-sans block whitespace-nowrap">
                                    {isBn ? `(মোট: ৳${r.amount.toLocaleString()})` : `(Total: ৳${r.amount.toLocaleString()})`}
                                  </span>
                                )}
                              </div>
                            );
                    ...
                    <tr className="bg-slate-200 font-black border-t-2 border-slate-800 text-xs">
                      <td colSpan={7} className="py-2.5 px-2 text-right border-r border-slate-400 whitespace-nowrap">
                        {isBn ? 'সর্বমোট প্রাপ্ত টাকা (Grand Total):' : 'Grand Total Received:'}
                      </td>
                      <td className="py-2.5 px-1.5 text-right font-mono font-black text-sm text-emerald-900 whitespace-nowrap">
                        {formatCurrency(totalReceived, isBn ? 'bn' : 'en')}
                      </td>
                    </tr>
                  </tbody>
              ...
              <div className="grid grid-cols-3 gap-6 pt-8 text-center text-xs print-signature-section">
          ...
          <div className="bg-white text-slate-950 rounded-2xl shadow-2xl border border-slate-300 w-full max-w-5xl max-h-[94vh] flex flex-col overflow-hidden print:border-none print:shadow-none print:max-w-none print:max-h-none print:w-full print:rounded-none print:overflow-visible print:block">
            ...
            <div className="p-6 sm:p-8 overflow-y-auto print:p-0 print:overflow-visible text-slate-950">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-100 p-4 rounded-xl border border-slate-400 text-xs mb-4">
                ...
                <div>
                  <span className="text-slate-600 block text-[11px] font-bold">{isBn ? 'সর্বমোট প্রাপ্ত টাকা:' : 'Total Received:'}</span>
                  <span className="font-black text-sm text-emerald-800 font-mono whitespace-nowrap">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 p-3 bg-slate-50 border border-slate-300 rounded-lg text-xs mb-5">
                ...
                <div className="text-[11px] font-mono font-bold text-slate-700 whitespace-nowrap">
              ...
              <div className="border-2 border-slate-800 rounded-lg overflow-hidden mb-6">
                <table className="w-full text-left text-xs border-collapse receipt-register-table table-fixed">
                  <thead>
                    <tr className="bg-slate-200 border-b-2 border-slate-800 text-slate-950 font-black text-[10px] sm:text-[11px] uppercase">
                      <th className="py-2 px-1 border-r border-slate-400 text-center w-[4%]">{isBn ? 'ক্র.' : 'Sl'}</th>
                      <th className="py-2 px-1.5 border-r border-slate-400 w-[13%] whitespace-nowrap">{isBn ? 'রিসিট নং' : 'Receipt No'}</th>
                      <th className="py-2 px-1 border-r border-slate-400 w-[10%] whitespace-nowrap">{isBn ? 'তারিখ' : 'Date'}</th>
                      <th className="py-2 px-1.5 border-r border-slate-400 w-[15%]">{isBn ? 'প্রকল্প' : 'Project'}</th>
                      <th className="py-2 px-1.5 border-r border-slate-400 w-[26%]">{isBn ? 'বিবরণ ও মাধ্যম' : 'Description & Mode'}</th>
                      <th className="py-2 px-1.5 border-r border-slate-400 w-[16%]">{isBn ? 'জমা অ্যাকাউন্ট' : 'Deposit Account'}</th>
                      <th className="py-2 px-1.5 text-right w-[16%] whitespace-nowrap">{isBn ? 'পরিমাণ (৳)' : 'Amount (Tk)'}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y border-slate-300 text-[10.5px]">
                    {contributorStatementItem.records.map((rec: MoneyReceived, idx: number) => (
                      <tr key={rec.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                        <td className="py-1.5 px-1 border-r border-slate-300 text-center font-bold">{idx + 1}</td>
                        <td className="py-1.5 px-1.5 border-r border-slate-300 font-mono font-bold text-slate-950 whitespace-nowrap">{rec.receiptNumber || 'MR-GEN'}</td>
                        <td className="py-1.5 px-1 border-r border-slate-300 whitespace-nowrap">{formatDisplayDate(rec.date, isBn ? 'bn' : 'en')}</td>
                        <td className="py-1.5 px-1.5 border-r border-slate-300 font-medium">{rec.projectName || 'General'}</td>
                        <td className="py-1.5 px-1.5 border-r border-slate-300">
                          <span className="font-semibold text-slate-900 block">{rec.description}</span>
                          <span className="text-slate-600 text-[9.5px]">{rec.paymentMethod}</span>
                        </td>
                        <td className="py-1.5 px-1.5 border-r border-slate-300 text-slate-700">{rec.accountName}</td>
                        <td className="py-1.5 px-1.5 text-right font-mono font-black text-slate-950 whitespace-nowrap">
                          {formatCurrency(rec.amount, isBn ? 'bn' : 'en')}
                        </td>
                      </tr>
                    ...
                    <tr className="bg-slate-200 font-black border-t-2 border-slate-800 text-xs">
                      <td colSpan={6} className="py-2.5 px-2 text-right border-r border-slate-400 whitespace-nowrap">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setEditAccountId(newAccId);
                      const acc = accounts.find(a => a.id === newAccId);
                      if (acc) {
                        if (acc.type === 'CASH') setEditPaymentMethod('CASH');
                        else if (acc.type === 'BANK') setEditPaymentMethod('BANK');
                        else if (acc.type === 'MOBILE_BANKING') setEditPaymentMethod('MOBILE_BANKING');
                      }
                    }}
                    required
                    className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-slate-100 outline-hidden font-medium cursor-pointer"
