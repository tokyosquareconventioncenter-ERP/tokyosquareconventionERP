...
export const ExpensesView: React.FC<ExpensesViewProps> = ({
  ...
  const { expenses, projects, accounts, vouchers, softDeleteExpense, updateExpense, selectedProjectId, contractors, suppliers } = useData();
  ...
  const openEditModal = (expense: Expense) => {
    ...
    setEditContractorId(expense.contractorId && expense.contractorId !== 'NONE' ? expense.contractorId : '');
    setEditSupplierId(expense.supplierId && expense.supplierId !== 'NONE' ? expense.supplierId : '');
  ...
  const handleSaveEdit = (e: React.FormEvent) => {
    ...
    updateExpense(editModalExpense.id, {
      ...
      paidTo: editPaidTo,
      contractorId: editExpenseType === 'CONTRACTOR' ? (editContractorId || undefined) : undefined,
      supplierId: editExpenseType === 'MATERIAL' ? (editSupplierId || undefined) : undefined,
      amount: editAmount,
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className={`border-b uppercase text-[10px] tracking-wider ${
                ...
                <th className="py-3 px-3 min-w-[135px]">{isBn ? 'ভাউচার নং ও এডিট' : 'Voucher No & Edit'}</th>
                <th className="py-3 px-3 min-w-[95px]">{isBn ? 'তারিখ' : 'Date'}</th>
                <th className="py-3 px-3 min-w-[150px]">{isBn ? 'প্রকল্প' : 'Project'}</th>
                <th className="py-3 px-3 min-w-[160px]">{isBn ? 'প্রাপক' : 'Paid To'}</th>
                <th className="py-3 px-3 min-w-[200px]">{isBn ? 'খাত ও বিবরণ' : 'Category & Details'}</th>
                <th className="py-3 px-3 min-w-[100px]">{isBn ? 'মাধ্যম' : 'Mode'}</th>
                <th className="py-3 px-3 text-right min-w-[110px]">{isBn ? 'টাকার পরিমাণ' : 'Amount'}</th>
                <th className={`py-3 px-3 text-center min-w-[130px] sticky right-0 z-20 shadow-[-6px_0_10px_-2px_rgba(0,0,0,0.08)] border-l ${
                      ...
                      <td className="py-3 px-3 whitespace-nowrap">
                        <div className="flex items-center gap-1.5">
                          ...
                          <button
                            ...
                            title={isBn ? 'সরাসরি খরচ এডিট করুন' : 'Quick Edit'}
                          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-xs p-3 overflow-y-auto">
          <div className="bg-slate-900 text-slate-100 rounded-2xl shadow-2xl border border-slate-700 w-full max-w-xl overflow-hidden my-6">
            ...
            <form onSubmit={handleSaveEdit} className="p-6 space-y-4 text-xs">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setEditExpenseType(newType);
                      if (newType === 'CONTRACTOR' && !editContractorId && contractors.length > 0) {
                        setEditContractorId(contractors[0].id);
                        setEditPaidTo(contractors[0].name);
                      }
                      if (newType === 'MATERIAL' && !editSupplierId && suppliers.length > 0) {
                        setEditSupplierId(suppliers[0].id);
                        setEditPaidTo(suppliers[0].name);
                      }
                    }}
                    className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-slate-100 outline-hidden font-medium cursor-pointer"
                  >
              ...
              {/* Conditional Contractor Selector */}
              {editExpenseType === 'CONTRACTOR' && (
                <div className="p-3 bg-amber-500/10 border border-amber-500/30 rounded-xl space-y-1">
                  <label className="block text-amber-400 font-bold text-xs">
                    👷 {isBn ? 'কন্ট্রাক্টর লিংক নির্বাচন করুন (খতিয়ানে যোগ হবে):' : 'Select Contractor Link (Will add to Ledger):'}
                  </label>
                  <select
                    value={editContractorId}
                    onChange={(e) => {
                      const cId = e.target.value;
                      setEditContractorId(cId);
                      const c = contractors.find(item => item.id === cId);
                      if (c) {
                        setEditPaidTo(c.name);
                        if (!editCategory || editCategory === 'যাতায়াত' || editCategory === 'Other') {
                          setEditCategory(`Contractor Payment (${c.type || 'General'})`);
                        }
                      }
                    }}
                    className="w-full bg-slate-800 border-2 border-amber-500/60 rounded-xl px-3 py-2 text-slate-100 font-semibold outline-hidden cursor-pointer"
                  >
                    <option value="">{isBn ? '-- কন্ট্রাক্টর নির্বাচন করুন --' : '-- Select Contractor --'}</option>
                    {contractors.map(c => (
              ...
              {/* Conditional Supplier Selector */}
              {editExpenseType === 'MATERIAL' && (
                <div className="p-3 bg-blue-500/10 border border-blue-500/30 rounded-xl space-y-1">
                  <label className="block text-blue-400 font-bold text-xs">
                    🏭 {isBn ? 'সরবরাহকারী প্রতিষ্ঠান নির্বাচন করুন (খতিয়ানে যোগ হবে):' : 'Select Supplier Link (Will add to Ledger):'}
                  </label>
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (s) {
                        setEditPaidTo(s.name);
                        if (!editCategory || editCategory === 'Other') {
                          setEditCategory(`সরবরাহকারী বিল পরিশোধ (${s.type || 'Material'})`);
                        }
                      }
                    }}
                    className="w-full bg-slate-800 border-2 border-blue-500/60 rounded-xl px-3 py-2 text-slate-100 font-semibold outline-hidden cursor-pointer"
                  >
                    <option value="">{isBn ? '-- সরবরাহকারী নির্বাচন করুন (ঐচ্ছিক) --' : '-- Select Supplier (Optional) --'}</option>
                    {suppliers.map(s => (
