...
export const UsersView: React.FC = () => {
  ...
  const handleSaveEdit = (e: React.FormEvent) => {
    ...
    setNotice(isBn 
      ? `ব্যবহারকারী "${editName}" এর তথ্য${editPassword.trim() ? ' এবং নতুন পাসওয়ার্ড' : ''} সফলভাবে আপডেট করা হয়েছে!` 
      : `User "${editName}" details${editPassword.trim() ? ' and password' : ''} updated successfully!`);
  };
            ...
            <form onSubmit={handleSaveEdit} className="space-y-3 text-xs sm:text-sm">
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">
                  {isBn ? 'নতুন পাসওয়ার্ড সেট করুন (ঐচ্ছিক)' : 'Set New Password (Optional)'}
                </label>
                <input
                  type="text"
                  placeholder={isBn ? 'পরিবর্তন না করতে চাইলে ফাঁকা রাখুন' : 'Leave empty to keep unchanged'}
                  value={editPassword}
                  onChange={(e) => setEditPassword(e.target.value)}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-hidden focus:border-purple-600 font-mono"
                />
                <p className="text-[11px] text-slate-500 mt-1">
                  {isBn ? 'পাসওয়ার্ড পরিবর্তন করতে চাইলে এখানে নতুন পাসওয়ার্ড লিখুন।' : 'Type a new password here if you want to update it.'}
                </p>
              </div>
