...
export const SKRP_MONTHLY_EXPENSE_HEADS = [
  'MD. eleyes',
  'Tutul',
  'Tutul Food Bill',
  'ফুড বিল (Food Bill)',
  'Electricity Bill',
  'সাইট বিদ্যুৎ বিল (Site Electricity Bill)',
  'Common Service Bill',
  'সকল প্রজেক্ট ও অফিস রক্ষণাবেক্ষণ (Project & Office Maintenance)',
  'Office Rent & Maintenance',
  'Internet & Stationary'
];

export const UNIVERSAL_MONTHLY_EXPENSE_HEADS = [
  'অফিস স্টাফ বেতন ও ভাতা (Staff Salary)',
  'সাইট ইঞ্জিনিয়ার সম্মানি (Site Engineer Allowance)',
  'অফিস ও সাইট বিদ্যুৎ বিল (Electricity Bill)',
  'অফিস ও সাইট রুম ভাড়া (Office & Site Rent)',
  'কর্মীদের খাবার ও নাস্তা বিল (Staff Food & Tea Bill)',
  'ইন্টারনেট, মোবাইল ও ইউটিলিটি বিল (Internet & Utility Bill)',
  'স্টেশনারি, ফটোকপি ও প্রিন্টিং (Stationery & Printing)',
  'সাইট যাতায়াত ও ফুয়েল বিল (Conveyance & Transport)',
  'সাইট সিকিউরিটি ও নাইট গার্ড বেতন (Security Guard Salary)',
  'কমন সার্ভিস ও সার্ভিস চার্জ (Common Service Bill)',
  'অফিস ও সাইট রক্ষণাবেক্ষণ (Office & Site Maintenance)'
];
...
const MONTHLY_HEADS_STORE_KEY = 'skrp_all_monthly_heads_v2';
const MONEY_SOURCES_STORE_KEY = 'skrp_all_money_sources_v2';
...
export function deleteCustomCategoryFromStorage(categoryName: string): string[] {
  const current = getSavedCustomCategories();
...
// ----------------- MONTHLY EXPENSE HEADS -----------------

export function getSavedCustomMonthlyExpenseHeads(): string[] {
  try {
    const data = localStorage.getItem(MONTHLY_HEADS_STORE_KEY);
    if (data) {
...
export function saveCustomMonthlyExpenseHeadToStorage(headName: string): string[] {
  ...
  if (!current.includes(trimmed)) {
    ...
    try {
      localStorage.setItem(MONTHLY_HEADS_STORE_KEY, JSON.stringify(updated));
    } catch (e) {
...
export function deleteMonthlyExpenseHeadFromStorage(headName: string): string[] {
  const current = getSavedCustomMonthlyExpenseHeads();
  const updated = current.filter(h => h !== headName);
  try {
    localStorage.setItem(MONTHLY_HEADS_STORE_KEY, JSON.stringify(updated));
  } catch (e) {
    console.error('Error deleting monthly expense head', e);
  }
  return updated;
}

export function resetMonthlyExpenseHeads(template: 'skrp' | 'universal'): string[] {
  const list = template === 'universal' ? UNIVERSAL_MONTHLY_EXPENSE_HEADS : SKRP_MONTHLY_EXPENSE_HEADS;
  try {
    localStorage.setItem(MONTHLY_HEADS_STORE_KEY, JSON.stringify(list));
  } catch (e) {
    console.error('Error resetting monthly expense heads', e);
  }
  return list;
}

// ----------------- MONEY RECEIVED SOURCES -----------------

export const SKRP_MONEY_SOURCES = [
  'Japan City Tower (জাপান সিটি টাওয়ার)',
  'Japan Garden City (জাপান গার্ডেন সিটি)',
  'Share Voucher / Partner (শেয়ার ভাউচার / অংশীদার)',
  'Owner (S.M. Khalilur Rahman / মালিক)',
  'Company Fund (কোম্পানি ফান্ড)',
  'Director / Executive (পরিচালক - Arifa Akhter, Hamid Sir)',
  'Client Booking (ক্লায়েন্ট ফ্ল্যাট বুকিং / প্রজেক্ট অগ্রিম)',
  'Loan / Borrowed (ব্যাংক লোন / ধার ঋণ)',
  'Other (অন্যান্য উৎস)'
];

export const UNIVERSAL_MONEY_SOURCES = [
  'মালিক / ব্যবস্থাপনা পরিচালক (Owner / MD)',
  'শেয়ার ভাউচার / অংশীদার বিনিয়োগ (Share / Partner)',
  'ক্লায়েন্ট ফ্ল্যাট বুকিং / প্রজেক্ট অগ্রিম (Client Booking / Advance)',
  'কোম্পানি সেন্ট্রাল ফান্ড (Company Central Fund)',
  'পরিচালক / এক্সিকিউটিভ (Director / Executive)',
  'ব্যাংক লোন / কর্জ ঋণ (Bank Loan / Borrowed)',
  'অন্যান্য উৎস (Other Source)'
];

export function getSavedMoneyReceivedSources(): string[] {
  try {
    const data = localStorage.getItem(MONEY_SOURCES_STORE_KEY);
    if (data) {
      const parsed = JSON.parse(data);
      if (Array.isArray(parsed) && parsed.length > 0) return parsed;
    }
    return SKRP_MONEY_SOURCES;
  } catch {
    return SKRP_MONEY_SOURCES;
  }
}

export function saveCustomMoneyReceivedSource(sourceName: string): string[] {
  const trimmed = sourceName.trim();
  if (!trimmed) return getSavedMoneyReceivedSources();

  const current = getSavedMoneyReceivedSources();
  if (!current.includes(trimmed)) {
    const updated = [...current, trimmed];
    try {
      localStorage.setItem(MONEY_SOURCES_STORE_KEY, JSON.stringify(updated));
    } catch (e) {
      console.error('Error saving custom money received source', e);
    }
    return updated;
  }
  return current;
...
export function deleteMoneyReceivedSource(sourceName: string): string[] {
  const current = getSavedMoneyReceivedSources();
  const updated = current.filter(s => s !== sourceName);
  try {
    localStorage.setItem(MONEY_SOURCES_STORE_KEY, JSON.stringify(updated));
  } catch (e) {
    console.error('Error deleting money received source', e);
  }
  return updated;
}

export function resetMoneyReceivedSources(template: 'skrp' | 'universal'): string[] {
  const list = template === 'universal' ? UNIVERSAL_MONEY_SOURCES : SKRP_MONEY_SOURCES;
  try {
    localStorage.setItem(MONEY_SOURCES_STORE_KEY, JSON.stringify(list));
  } catch (e) {
    console.error('Error resetting money sources', e);
  }
  return list;
}
